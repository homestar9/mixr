...
**Vite version compatibility.** The `.vite/manifest.json` schema is a stable, documented backend-integration contract that is unchanged from Vite 5 through Vite 8 (Vite 8's Rolldown bundler, Oxc transforms, and Lightning CSS minification are build-engine internals that do not change manifest output). The driver reads only `file`, `css`, and `imports` from each node and ignores every other field (`name`, `src`, `isEntry`, `isDynamicEntry`, `dynamicImports`, `assets`, `integrity`), so newer Vite versions are non-breaking. `dynamicImports` (lazy chunks) are intentionally NOT walked — they must not be pulled into the eager `css[]`/modulepreload graph. See the `Vite 8 manifest tolerance` specs in `ViteDriverTest` and the `manifest-vite8.json` fixture.
